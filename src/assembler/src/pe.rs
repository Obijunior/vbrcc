...
// This is a minimal implementation to allow the assembler to produce .o files that can be linked

pub fn create_pe_wrapper(text_code: &[u8], data_content: &[u8]) -> Vec<u8> {
    let mut pe = Vec::new();
    ...
    pe.extend_from_slice(&2u16.to_le_bytes());      // Sections: .text and .data
    ...
    // Image Size: Headers (0x1000) + .text (aligned) + .data (aligned)
    let text_aligned = (text_code.len() as u32 + 0xFFF) & !0xFFF;
    let data_aligned = (data_content.len() as u32 + 0xFFF) & !0xFFF;
    let size_of_image = 0x1000 + text_aligned + data_aligned;
    ...
    pe.extend_from_slice(&(data_content.len() as u32).to_le_bytes());
    let data_raw_ptr = 0x400 + ((text_code.len() as u32 + 0x1FF) & !0x1FF);
